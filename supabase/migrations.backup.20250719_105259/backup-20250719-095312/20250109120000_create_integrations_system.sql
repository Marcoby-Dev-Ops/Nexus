...
-- Ensure integrations table has all required columns
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns WHERE table_name='integrations' AND column_name='auth_type'
    ) THEN
        ALTER TABLE public.integrations ADD COLUMN auth_type TEXT NOT NULL DEFAULT 'api_key';
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns WHERE table_name='integrations' AND column_name='features'
    ) THEN
        ALTER TABLE public.integrations ADD COLUMN features JSONB DEFAULT '[]';
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns WHERE table_name='integrations' AND column_name='estimated_setup_time'
    ) THEN
        ALTER TABLE public.integrations ADD COLUMN estimated_setup_time TEXT DEFAULT '10 min';
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns WHERE table_name='integrations' AND column_name='difficulty'
    ) THEN
        ALTER TABLE public.integrations ADD COLUMN difficulty TEXT DEFAULT 'medium';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_integration_data_updated_at') THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'integrations' AND policyname = 'Everyone can view active integrations'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_integrations' AND policyname = 'Users can view own integrations'
    ) THEN
