...
function getSystemMessage(
  userProfile: { name: string; company?: string },
  ragContext: string,
  summaryChunks: string[]
): string {
  // Simple system prompt with context
  return [
    `You are an AI assistant for ${userProfile.company || "the user"}.`,
    ragContext ? `Context:\n${ragContext}` : "",
    summaryChunks.length ? `Summary:\n${summaryChunks.join("\n")}` : "",
  ]
    .filter(Boolean)
    .join("\n\n");
}

function createDataChunk(chunk: string): Uint8Array {
  // For streaming: format as SSE data chunk
...
async function getAIResponse(
  ...
  // Minimal: call OpenAI/compatible API, stream or return the result
  const messages = [
...
function createErrorChunk(message: string): Uint8Array {
  // For streaming: format error as SSE data chunk
