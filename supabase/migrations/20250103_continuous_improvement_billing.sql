...
CREATE TABLE user_billing_plans (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_ai_user_feedback_user_id ON ai_user_feedback(user_id);
CREATE INDEX idx_ai_user_feedback_agent_id ON ai_user_feedback(agent_id);
CREATE INDEX idx_ai_user_feedback_timestamp ON ai_user_feedback(timestamp);
CREATE INDEX idx_ai_user_feedback_rating ON ai_user_feedback(rating);

CREATE INDEX idx_ai_quality_assessments_overall_score ON ai_quality_assessments(overall_score);
CREATE INDEX idx_ai_quality_assessments_timestamp ON ai_quality_assessments(timestamp);
CREATE INDEX idx_ai_quality_assessments_agent_id ON ai_quality_assessments(agent_id);

CREATE INDEX idx_ai_improvement_recommendations_priority ON ai_improvement_recommendations(priority);
CREATE INDEX idx_ai_improvement_recommendations_status ON ai_improvement_recommendations(status);
CREATE INDEX idx_ai_improvement_recommendations_type ON ai_improvement_recommendations(type);
...
CREATE INDEX idx_ai_cost_allocations_user_id ON ai_cost_allocations(user_id);
CREATE INDEX idx_ai_cost_allocations_timestamp ON ai_cost_allocations(timestamp);
CREATE INDEX idx_ai_cost_allocations_billing_category ON ai_cost_allocations(billing_category);
CREATE INDEX idx_ai_cost_allocations_agent_id ON ai_cost_allocations(agent_id);

CREATE INDEX idx_ai_billing_records_user_id ON ai_billing_records(user_id);
CREATE INDEX idx_ai_billing_records_billing_period ON ai_billing_records(billing_period_start, billing_period_end);
CREATE INDEX idx_ai_billing_records_status ON ai_billing_records(status);

CREATE INDEX idx_ai_performance_metrics_metric_type ON ai_performance_metrics(metric_type);
CREATE INDEX idx_ai_performance_metrics_timestamp ON ai_performance_metrics(timestamp);
CREATE INDEX idx_ai_performance_metrics_agent_id ON ai_performance_metrics(agent_id);
...
('free', 'Free Plan', 'free', 0, 10000, 0.002, 
 ARRAY['Basic AI chat', 'Limited models', 'Community support'], 
 '{"maxRequestsPerHour": 20, "maxRequestsPerDay": 100, "maxConcurrentRequests": 1, "maxTokensPerRequest": 1000}',
 '{"openai": false, "openrouter": true, "premiumModels": false}'),
('pro', 'Pro Plan', 'pro', 29.99, 100000, 0.0015, 
 ARRAY['Advanced AI chat', 'All models', 'Priority support', 'Analytics dashboard'], 
 '{"maxRequestsPerHour": 200, "maxRequestsPerDay": 2000, "maxConcurrentRequests": 5, "maxTokensPerRequest": 4000}',
 '{"openai": true, "openrouter": true, "premiumModels": true}'),
('enterprise', 'Enterprise Plan', 'enterprise', 99.99, 500000, 0.001, 
 ARRAY['Enterprise AI chat', 'All models', 'Dedicated support', 'Advanced analytics', 'Custom integrations'], 
 '{"maxRequestsPerHour": 1000, "maxRequestsPerDay": 10000, "maxConcurrentRequests": 20, "maxTokensPerRequest": 8000}',
 '{"openai": true, "openrouter": true, "premiumModels": true}');
