...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = safe_rename_column.table_name
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        AND table_name = safe_rename_column.table_name
        AND column_name = safe_rename_column.old_column_name
    ) THEN
        RAISE NOTICE 'Column %.% does not exist, skipping rename', table_name, old_column_name;
        RETURN FALSE;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE NOTICE 'Column %.% already exists, skipping rename', table_name, new_column_name;
        RETURN FALSE;
    END IF;
    ...
    RAISE NOTICE 'Successfully renamed column %.% to %.%', 
                 table_name, old_column_name, table_name, new_column_name;
    RETURN TRUE;
END;
$$ LANGUAGE plpgsql;
...
-- Update the contacts policy to use the new column name
...
-- Update the deals policy to use the new column name
