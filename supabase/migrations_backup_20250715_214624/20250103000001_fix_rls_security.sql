...
ALTER TABLE IF EXISTS contacts ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES companies(id) ON DELETE CASCADE;
...
-- Add company_id column to deals table if it doesn't exist
ALTER TABLE IF EXISTS deals ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES companies(id) ON DELETE CASCADE;
