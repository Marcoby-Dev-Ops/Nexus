...
// Instantiate services (stateless-ish service objects)
...
router.get('/user-mapping', authenticateToken, async (req, res) => {
  try {
    ...
    let profileExists = profile && profile.length > 0;
    ...
    // If profile doesn't exist yet, provision it using Authentik attributes
    if (!profileExists) {
      try {
        // Fetch user details from Authentik
        const detailsResp = await fetch(`${AUTHENTIK_BASE_URL}/api/v3/core/users/${userId}/`, {
          headers: { 'Authorization': `Bearer ${AUTHENTIK_API_TOKEN}` }
        });

        if (detailsResp.ok) {
          const userData = await detailsResp.json();

          // Build a minimal signupData object from Authentik attributes
          const attrs = userData.attributes || {};
          const signupData = {
            businessName: attrs.business_name || attrs.company || attrs.businessName || null,
            businessType: attrs.business_type || null,
            industry: attrs.industry || null,
            companySize: attrs.company_size || attrs.companySize || null,
            firstName: attrs.first_name || attrs.firstName || null,
            lastName: attrs.last_name || attrs.lastName || null,
          ...
          // Ensure user profile exists in local DB
          const ensureProfile = await userProfileService.ensureUserProfile(userId, signupData.email, {
          ...
          if (!ensureProfile.success) {
            logger.warn('Failed to create user profile during provisioning', { userId, error: ensureProfile.error });
          }

          // Ensure company exists and is associated
          const ensureCompany = await companyService.ensureCompanyForUser(userId, signupData);
          if (!ensureCompany.success) {
            logger.warn('Failed to create company during provisioning', { userId, error: ensureCompany.error });
          } else if (ensureCompany.company && ensureCompany.created) {
            // Associate user with company (company_id in user_profiles)
            await companyService.associateUserWithCompany(userId, ensureCompany.company.id, 'owner');
          }

          // Re-query profile to return the newly created profile
          const { data: newProfile, error: newProfileErr } = await query(profileQuery, [userId]);
          if (!newProfileErr && newProfile && newProfile.length > 0) {
            profileExists = true;
            profile[0] = newProfile[0];
          }
        } else {
          logger.warn('Failed to fetch Authentik details during provisioning', { userId });
        }
      } catch (provErr) {
        logger.error('Provisioning error for user during user-mapping', { userId, error: provErr?.message || provErr });
      }
    }
