...
const factsLatencyHistogram = promClient ? new promClient.Histogram({ name: 'rpc_match_facts_latency_seconds', help: 'Latency for match_facts rpc in seconds', buckets: [0.01, 0.05, 0.1, 0.5, 1, 2] }) : null;
const factsMatchesCounter = promClient ? new promClient.Counter({ name: 'rpc_match_facts_matches_total', help: 'Number of matches returned by match_facts' }) : null;
  ...
  try {
    ...
    switch (functionName) {
      case 'match_documents':
        ...
        break;
      case 'match_facts':
        result = await matchFacts(params, userId);
        break;
      ...
      case 'get_company_next_action':
        ...
        break;
        case 'search_users': {
          // Admin-only search across user_profiles
          const q = (params && params.q) ? String(params.q).trim() : '';

          // Only allow admins or owners to search all users
          const allowedRoles = ['admin', 'owner'];
          if (!allowedRoles.includes(req.user.role)) {
            throw createError('Unauthorized: insufficient role to search users', 403);
          }

          // Build a safe parameterized query to search by name or email
          const likeParam = `%${q.replace(/%/g, '\\%').replace(/_/g, '\\_')}%`;

          const sql = `
            SELECT id, user_id, external_user_id, first_name, last_name, full_name, display_name, email, business_email, role, company_id, created_at, updated_at
            FROM user_profiles
            WHERE (
              COALESCE(full_name, '') ILIKE $1 OR
              COALESCE(first_name || ' ' || last_name, '') ILIKE $1 OR
              COALESCE(email, '') ILIKE $1 OR
              COALESCE(business_email, '') ILIKE $1
            )
            ORDER BY created_at DESC
            LIMIT 1000
          `;

          const resultRows = await query(sql, [likeParam], req.user.jwtPayload);

          if (resultRows.error) {
            throw createError(`search_users failed: ${resultRows.error}`, 500);
          }

          result = resultRows.data || [];
        }
          break;
...
/**
 * Match facts by vector similarity
 */
async function matchFacts(params, userId) {
  const start = Date.now();
  const { query_embedding, match_count = 5, org_id = null, filter = {} } = params;
  ...
  const sqlParams = [query_embedding];
  let paramIndex = 2;

  if (org_id) {
    sql += ` AND org_id = $${paramIndex}`;
    sqlParams.push(org_id);
    paramIndex++;
  }
  ...
  if (result.error) {
    throw createError(`match_facts failed: ${result.error}`, 500);
  }

  // Telemetry: compute avg score and record latency
  try {
    const rows = result.data || [];
    const avgScore = rows.length ? (rows.reduce((s, r) => s + (r.score || 0), 0) / rows.length) : 0;
    const durationMs = Date.now() - start;
    logger.info('match_facts executed', { userId, org_id, match_count, resultCount: rows.length, avgScore, durationMs });
    if (factsLatencyHistogram) factsLatencyHistogram.observe(durationMs / 1000);
    if (factsMatchesCounter) factsMatchesCounter.inc(rows.length || 0);
  } catch (e) {
    logger.warn('Telemetry for match_facts failed', e?.message || e);
  }
