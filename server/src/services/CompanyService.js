...
class CompanyService {
    ...
    try {
      ...
      const companyResult = await query(
        `INSERT INTO companies (
          ...
          business_type, funding_stage, revenue_range,
  ...
  /**
   * Patch existing company with new business attributes (idempotent / additive)
   * Accepts same shape as signupData used in ensureCompanyForUser.
   */
  async patchBusinessAttributes(companyId, attrs = {}, jwtPayload = null) {
    try {
      this.logger.info('Patching company business attributes', { companyId, attrs });

      const { data: existingRows, error: existingError } = await query(
        'SELECT id, industry, size, description, website, domain, settings FROM companies WHERE id = $1',
        [companyId],
        jwtPayload
      );
      if (existingError) throw new Error(existingError);
      if (!existingRows || existingRows.length === 0) {
        return { success: false, updated: false, error: 'Company not found' };
      }
  const existing = existingRows[0];

      // Determine top-level updates (only apply if provided & non-empty)
      const updates = {};
      if (attrs.businessName && attrs.businessName !== existing.name && existing.name === 'My Business') {
        updates.name = attrs.businessName;
      } else if (attrs.businessName && attrs.businessName !== existing.name && existing.name && existing.name.length <= 3) {
        // Extremely short placeholder name
        updates.name = attrs.businessName;
      }
  if (attrs.industry && attrs.industry !== existing.industry) updates.industry = attrs.industry;
      if (attrs.companySize && attrs.companySize !== existing.size) updates.size = attrs.companySize;
      if (attrs.website && attrs.website !== existing.website) updates.website = attrs.website;
      if (attrs.domain && attrs.domain !== existing.domain) updates.domain = attrs.domain;
      if (attrs.description && attrs.description !== existing.description) updates.description = attrs.description;
  if (attrs.businessType && attrs.businessType !== existing.business_type) updates.business_type = attrs.businessType;
  if (attrs.fundingStage && attrs.fundingStage !== existing.funding_stage) updates.funding_stage = attrs.fundingStage;
  if (attrs.revenueRange && attrs.revenueRange !== existing.revenue_range) updates.revenue_range = attrs.revenueRange;

      // Merge business metadata into settings JSON
      const settings = existing.settings || {};
      const mergedSettings = {
        ...settings,
        business_type: attrs.businessType ?? settings.business_type,
        funding_stage: attrs.fundingStage ?? settings.funding_stage,
        revenue_range: attrs.revenueRange ?? settings.revenue_range
      };

      // Only include settings if changed
      const settingsChanged = JSON.stringify(settings) !== JSON.stringify(mergedSettings);
      if (settingsChanged) updates.settings = JSON.stringify(mergedSettings);

      if (Object.keys(updates).length === 0) {
        return { success: true, updated: false, company: existing };
      }
      ...
      const { data: updatedRows, error: updateError } = await query(
        `UPDATE companies SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $1 RETURNING *`,
        values,
        jwtPayload
      );
      if (updateError) throw new Error(updateError);
      return { success: true, updated: true, company: updatedRows[0] };
    } catch (error) {
      this.logger.error('Failed to patch business attributes', { companyId, error: error.message });
  ...
  async getBusinessIdentity(companyId, jwtPayload = null) {
    try {
      ...
      const result = await query(
        'SELECT business_identity, name FROM companies WHERE id = $1 AND is_active = true',
        [companyId],
        jwtPayload
      );
      ...
      // If business_identity exists, use it; otherwise start with an empty object.
  const row = result.data[0];
  const businessIdentity = row.business_identity || {};
  // Prefer foundation_name column if present (new migration); otherwise fallback
  const companyName = row.foundation_name || row.name || 'My Company';

  // Ensure foundation object exists and has a sensible name fallback
  businessIdentity.foundation = businessIdentity.foundation || {};
  businessIdentity.foundation.name = businessIdentity.foundation.name || companyName;
      // Ensure other sub-objects exist to avoid callers assuming structure
      businessIdentity.missionVisionValues = businessIdentity.missionVisionValues || {};
      businessIdentity.productsServices = businessIdentity.productsServices || {};
      businessIdentity.targetMarket = businessIdentity.targetMarket || {};
      businessIdentity.competitiveLandscape = businessIdentity.competitiveLandscape || {};
      businessIdentity.businessOperations = businessIdentity.businessOperations || {};
      businessIdentity.financialContext = businessIdentity.financialContext || {};
      businessIdentity.strategicContext = businessIdentity.strategicContext || {};
  ...
  const updatedIdentity = this.deepMerge(currentIdentity, updates || {});
      ...
      // Additionally, sync key foundation fields into top-level company columns
      try {
        const foundation = updatedIdentity.foundation || {};
        const fields = [];
        const values = [];
        let idx = 1;

        if (foundation.name) {
          fields.push(`foundation_name = $${idx++}`);
          values.push(foundation.name);
          // also keep top-level name in sync if present
          fields.push(`name = $${idx++}`);
          values.push(foundation.name);
        }

        if (foundation.industry) {
          fields.push(`industry = $${idx++}`);
          values.push(foundation.industry);
        }

        // companySize may be in several shapes; try common keys
        const companySize = foundation.companySize || updatedIdentity.companySize || updatedIdentity.size;
        if (companySize) {
        ...
        if (foundation.website) {
          ...
          values.push(foundation.website);
        }

        if (foundation.headquarters && Object.keys(foundation.headquarters).length > 0) {
          fields.push(`address = $${idx++}`);
          values.push(JSON.stringify(foundation.headquarters));
        }

        // contact_info: try to capture email/phone/legal fields
        const contact = {};
        if (foundation.email) contact.email = foundation.email;
        if (foundation.phone) contact.phone = foundation.phone;
        if (foundation.legalName) contact.legalName = foundation.legalName;
        if (foundation.legalStructure) contact.legalStructure = foundation.legalStructure;
        if (Object.keys(contact).length > 0) {
          fields.push(`contact_info = COALESCE(contact_info, '{}'::jsonb) || $${idx++}::jsonb`);
          values.push(JSON.stringify(contact));
        }

        if (fields.length > 0) {
          values.push(companyId);
          const sql = `UPDATE companies SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${idx}`;
          const colUpdate = await query(sql, values, jwtPayload);
          if (colUpdate.error) {
            this.logger.warn('Failed to sync business_identity fields to company columns', { companyId, error: colUpdate.error });
          } else {
            this.logger.info('Synced business_identity fields to company columns', { companyId });
          }
        }
      } catch (syncErr) {
        this.logger.warn('Exception while syncing identity fields to company columns', { companyId, error: syncErr.message });
      }
