...
async function generateSystemPrompt(agentId, context) {
  ...
  // Try to get a shared/tenant personality (falls back to default if not present)
  let sharedPersonality = '';
  try {
    const tenantId = context?.tenantId || context?.user?.tenantId || null;
    if (tenantId) {
      const res = await query('SELECT personality_blob FROM tenant_personalities WHERE tenant_id = $1 AND is_active = true LIMIT 1', [tenantId]);
      if (res && res.rows && res.rows[0] && res.rows[0].personality_blob) {
        sharedPersonality = res.rows[0].personality_blob;
      }
    }
  } catch (err) {
    logger.warn('Unable to fetch shared personality, continuing without it', { err: err?.message });
  }
  ...
  // If there's a shared personality, place it before the selected prompt so it applies globally
  let systemPrompt = `${sharedPersonality ? `SHARED PERSONALITY:\n${sharedPersonality}\n\n` : ''}${selectedPrompt}
