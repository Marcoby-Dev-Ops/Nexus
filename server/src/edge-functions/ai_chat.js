...
const { registerRequest, unregisterRequest } = require('./ai_request_registry');
 ...
 */
async function generateAIResponse(message, systemPrompt, conversationHistory, agentId = 'assistant', businessHealth = {}, signal) {
  try {
...
async function ai_chat(payload, user) {
  ...
  try {
    ...
    // If client provided a requestId, register an AbortController so it can be cancelled via the ai_abort edge function
    const requestId = payload.requestId || null;
    let controller = null;
    if (requestId) {
      const { AbortController } = globalThis || global || window || {};
      try {
    ...
    // Generate AI response, passing along controller.signal when available
    ...
    // Normalize final content (aiResponse may be a string or an object)
    const finalContent = typeof aiResponse === 'string' ? aiResponse : (aiResponse?.content || JSON.stringify(aiResponse));

    // Persist assistant message to DB if we have a conversationId
    let savedMessageRow = null;
    try {
      if (conversationId) {
        const metadata = JSON.stringify({ tokens: Math.ceil((finalContent || '').length / 4), model: 'gpt-4', streaming: false });
        const now = new Date().toISOString();
        const insertSql = `INSERT INTO ai_messages (conversation_id, role, content, metadata, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`;
        const insertParams = [conversationId, 'assistant', finalContent, metadata, now, now];
        const insertResult = await query(insertSql, insertParams);
        if (!insertResult.error && insertResult.data && insertResult.data[0]) {
          savedMessageRow = insertResult.data[0];
        }
      }
    } catch (err) {
      logger.warn('Failed to persist assistant message', { error: err instanceof Error ? err.message : err });
      savedMessageRow = null;
    }
    ...
    try {
      const requestId = payload.requestId || null;
      if (requestId) unregisterRequest(requestId);
    } catch (e) {
      // ignore
    }
  }
