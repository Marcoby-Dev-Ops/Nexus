...
const { 
  generalLimiter, 
  authLimiter, 
  authTokenLimiter, oauthStateLimiter,
  dbLimiter, 
  aiLimiter, 
  uploadLimiter, 
  devLimiter 
...
// Import Socket.IO test routes (development only)
let socketTestRoutes = null;
if (process.env.NODE_ENV !== 'production') {
  try {
    socketTestRoutes = require('./src/in-progress/routes/socket-test');
  } catch (error) {
    logger.warn('Socket.IO test routes unavailable', { error: error.message });
  }
}
...
// OAuth routes: apply authLimiter to sensitive endpoints, but skip lightweight ones (/state, /userinfo)
// OAuth routes refined rate limiting: skip lightweight, special limiter for /token
app.use('/api/oauth', (req, res, next) => {
  const path = req.path.replace(/\/+$/, ''); // trim trailing slash
  if (path === '/state') {
    return oauthStateLimiter(req, res, next);
  }
  if (path === '/userinfo') {
    return next(); // exempt; inexpensive and often needed after token
  }
  if (path === '/token') {
    return authTokenLimiter(req, res, next);
  }
