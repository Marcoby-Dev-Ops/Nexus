# Nexus Docs Index

## Start Here

- [Project README](../README.md)
- [Architecture and Philosophy](ARCHITECTURE_AND_PHILOSOPHY.md)
- [Project Context](PROJECT_CONTEXT.md)
- [Engineering Status](current/ENGINEERING_STATUS.md)

## Core Technical Docs

- [Service Layer Architecture](SERVICE_LAYER_ARCHITECTURE.md)
- [OpenClaw Integration](OPENCLAW_INTEGRATION.md)
- [RAG System Architecture](architecture/RAG_SYSTEM_ARCHITECTURE.md)
- [AI Capabilities Summary](implementation/AI_CAPABILITIES_SUMMARY.md)
- [Deployment Guide](deployment/DEPLOYMENT.md)
- [Testing Checklist](testing/TESTING_CHECKLIST.md)

## Development Guides

- [Developer Guide](DEVELOPER_GUIDE.md)
- [Quick Start Guide](QUICK_START_GUIDE.md)
- [Getting Started (Dev)](GETTING_STARTED_DEV.md)
- [PR Template](PR_TEMPLATE.md)
- [OAuth Guide](development/OAUTH_GUIDE.md)

## Current Operational Docs

- [Engineering Status](current/ENGINEERING_STATUS.md)
- [Database Retirement Plan](current/DATABASE_RETIREMENT_PLAN.md)

## Archived Docs

- [MVP Archive](archive/mvp/README.md)
- [Legacy Planning Archive](archive/legacy-planning/README.md)
- [Legacy Database Archive](archive/legacy-database/README.md)

## Notes

- This index intentionally links only to active, verified docs.
- Historical and superseded content remains in `docs/archive/`.
