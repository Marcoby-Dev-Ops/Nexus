...
export async function getAuthHeaders(): Promise<Record<string, string>> {
  ...
  if (sessionData) {
    try {
      ...
      if (session?.session?.accessToken) {
        ...
        // Only log in development and when debugging auth issues
  ...
  if (!token) {
    ...
    for (const key of tokenKeys) {
      ...
      if (storedToken) {
        token = storedToken;
        // Only log in development and when debugging auth issues
        loggingUtils.auth(`Found token in ${key}`);
        break;
      }
    }
  }
  ...
  if (token) {
    ...
    // Only log in development and when debugging auth issues
    loggingUtils.auth('Authorization header set');
  } else {
    // Only log in development and when debugging auth issues
    loggingUtils.auth('No token found, requests will be unauthenticated');
    // Show a helpful message to the user
    if (typeof window !== 'undefined' && !window.location.pathname.includes('/auth/') && !window.location.pathname.includes('/login')) {
      console.warn('User not authenticated. Please sign in to access the application.');
    }
  }
