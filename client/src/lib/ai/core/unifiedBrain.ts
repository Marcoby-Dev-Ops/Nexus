...
import { agentRegistry, type Agent } from './agentRegistry';
import { ContextualRAG } from './contextualRAG';
import { AIAgentWithTools } from '@/services/ai/aiAgentWithTools';
...
export class UnifiedBrain {
  ...
  private aiAgentWithTools: AIAgentWithTools;
  ...
  constructor() {
    ...
    this.aiAgentWithTools = new AIAgentWithTools();
  ...
  private async selectOptimalAgent(
    ...
    context: UnifiedBrainContext
  ): Promise<{ agent: Agent; confidence: number; reasoning: string }> {
    
    // Default to executive assistant
    let selectedAgent = agentRegistry.executiveAgent;
    let confidence = 0.7;
    ...
    return { agent: selectedAgent, confidence, reasoning };
  }
  ...
  private async executeUnifiedBrainPipeline(
    request: UnifiedBrainRequest,
    context: UnifiedBrainContext,
    agentSelection: { agent: Agent; confidence: number; reasoning: string },
  ...
  private async routeChatRequest(request: UnifiedBrainRequest, context: UnifiedBrainContext, analysis: any): Promise<Agent> {
    // Route chat requests to appropriate agent
    return agentRegistry.executiveAgent;
  }

  private async routeActionRequest(request: UnifiedBrainRequest, context: UnifiedBrainContext, analysis: any): Promise<Agent> {
    // Route action requests to appropriate agent
    return agentRegistry.executiveAgent;
  }

  private async routeGenerationRequest(request: UnifiedBrainRequest, context: UnifiedBrainContext, analysis: any): Promise<Agent> {
    // Route generation requests to appropriate agent
    return agentRegistry.executiveAgent;
  }

  private async routeAnalysisRequest(request: UnifiedBrainRequest, context: UnifiedBrainContext, analysis: any): Promise<Agent> {
    // Route analysis requests to appropriate agent
    return agentRegistry.executiveAgent;
  }

  private async routeAutomationRequest(request: UnifiedBrainRequest, context: UnifiedBrainContext, analysis: any): Promise<Agent> {
    // Route automation requests to appropriate agent
    return agentRegistry.executiveAgent;
  }

  private calculateAgentConfidence(agent: Agent, request: UnifiedBrainRequest, context: UnifiedBrainContext): number {
...
AGENT CAPABILITIES:
- Expertise: ${agent.knowledgeBase.expertise.join(', ')}
- Tools: ${agent.knowledgeBase.tools.join(', ')}
- Frameworks: ${agent.knowledgeBase.frameworks.join(', ')}
