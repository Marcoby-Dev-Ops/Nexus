...
export class CentralizedRLSService extends BaseService {
  ...
  async getTablePolicyInfo(): Promise<ServiceResponse<TablePolicyInfo[]>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<TablePolicyInfo[]>('get_table_policy_info', {});
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to get table policy info');
        return { data: resp.data || [], error: null, success: true };
      }, 'get_table_policy_info');
    } catch (error) {
  ...
  async getPolicySummary(): Promise<ServiceResponse<PolicySummary[]>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<PolicySummary[]>('list_policy_summary', {});
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to get policy summary');
        return { data: resp.data || [], error: null, success: true };
      }, 'list_policy_summary');
    } catch (error) {
  ...
  async validatePolicyCoverage(): Promise<ServiceResponse<PolicyCoverage[]>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<PolicyCoverage[]>('validate_policy_coverage', {});
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to validate policy coverage');
        return { data: resp.data || [], error: null, success: true };
  ...
  async applyUserLevelPolicies(tableName: string): Promise<ServiceResponse<void>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('apply_user_level_policies', { table_name: tableName });
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to apply user-level policies');
        this.logger.info(`Applied user-level policies to table: ${tableName}`);
        return { data: null, error: null, success: true };
      }, `apply_user_level_policies ${tableName}`);
    } catch (error) {
  ...
  async applyCompanyLevelPolicies(tableName: string): Promise<ServiceResponse<void>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('apply_company_level_policies', { table_name: tableName });
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to apply company-level policies');
        this.logger.info(`Applied company-level policies to table: ${tableName}`);
        return { data: null, error: null, success: true };
      }, `apply_company_level_policies ${tableName}`);
    } catch (error) {
  ...
  async applyHybridPolicies(tableName: string): Promise<ServiceResponse<void>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('apply_hybrid_policies', { table_name: tableName });
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to apply hybrid policies');
        this.logger.info(`Applied hybrid policies to table: ${tableName}`);
        return { data: null, error: null, success: true };
      }, `apply_hybrid_policies ${tableName}`);
    } catch (error) {
  ...
  async applyReadOnlyPolicies(tableName: string): Promise<ServiceResponse<void>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('apply_readonly_policies', { table_name: tableName });
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to apply read-only policies');
        this.logger.info(`Applied read-only policies to table: ${tableName}`);
        return { data: null, error: null, success: true };
      }, `apply_readonly_policies ${tableName}`);
    } catch (error) {
  ...
  async applyPoliciesToNewTable(tableName: string, policyType: string): Promise<ServiceResponse<void>> {
    try {
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('apply_policies_to_new_table', { table_name: tableName, policy_type: policyType });
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to apply policies to new table');
        this.logger.info(`Applied ${policyType} policies to new table: ${tableName}`);
        return { data: null, error: null, success: true };
      }, `apply_policies_to_new_table ${tableName}`);
    } catch (error) {
  ...
  async migrateToCentralizedSystem(): Promise<ServiceResponse<void>> {
    try {
      ...
      return await this.executeDbOperation(async () => {
        const resp = await callRPC<void>('migrate_to_centralized_rls', {});
        if (!resp || !resp.success) throw new Error(resp?.error || 'Failed to migrate to centralized RLS');
        this.logger.info('Successfully migrated to centralized RLS system');
        return { data: null, error: null, success: true };
      }, 'migrate_to_centralized_rls');
    } catch (error) {
  ...
  async getTablesNeedingAttention(): Promise<ServiceResponse<TablePolicyInfo[]>> {
    try {
      ...
      // We need to return TablePolicyInfo[], so fetch table info and join on tableName
      const tableInfoResult = await this.getTablePolicyInfo();
      if (!tableInfoResult.success) {
        throw new Error('Failed to get table policy info');
      }

      const failingNames = (coverageResult.data || [])
        .filter(c => c.status === 'missing' || c.status === 'partial' || c.status === 'error')
        .map(c => c.tableName);

      const tablesNeedingAttention = (tableInfoResult.data || []).filter(t => failingNames.includes(t.tableName));
