...
export default function Signup() {
  ...
  useEffect(() => {
    ...
    // Small heuristic: only check session-info when there's reason to believe
    // the user may already have an authenticated enrollment session.
    const maybeEnrollment = !!(
      email ||
      username ||
      urlParams.get('enrollment') === '1'
    );

    // Non-httpOnly indicator cookie (development/enrollment helper). In production
    // prefer URL params or server-side handling. This check is best-effort and
    // will quietly fail if cookies aren't accessible in the browser.
    let hasEnrollmentIndicator = false;
    try {
      hasEnrollmentIndicator = typeof document !== 'undefined' && document.cookie.indexOf('nexus_enrollment=1') !== -1;
    } catch (e) {
      hasEnrollmentIndicator = false;
    }
    ...
    const checkEnrollmentUser = async () => {
      try {
        ...
        // Treat 401 (not authenticated) as a normal outcome and continue
        if (sessionResponse.status === 401) {
          // no session, continue normal signup flow
          return;
        }

        if (!sessionResponse.ok) {
          // other non-OK responses are logged for debugging but don't block signup
          console.warn('session-info returned non-ok status', sessionResponse.status);
          return;
        }
        ...
        // Network or other errors should not block the signup flow.
        // Keep this debug-level to avoid alarming normal users.
        console.debug('session-info check failed, continuing signup flow', error);
      }
    };
    ...
    if (email || username) {
      ...
      // No URL parameters: only check session when there's reason to believe an
      // authenticated enrollment session might exist.
      if (maybeEnrollment || hasEnrollmentIndicator) {
        checkEnrollmentUser();
      }
    }
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <OptimizedSignupField
          ...
          autoComplete="tel"
          inputMode="tel"
          // Pattern: optional leading +, digits, spaces, hyphens or parentheses
