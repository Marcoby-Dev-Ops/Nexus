...
export default function AuthentikAuthCallback() {
  ...
  // Minimal mount log (retain one line for diagnostics; remove verbose params)
  logger.info('🔍 [MarcobyIAMCallback] Mounted');

  useEffect(() => {
    const handleCallback = async () => {
  logger.info('🔍 [MarcobyIAMCallback] Processing callback...');
      
      // Prevent multiple processing
      // In dev (React StrictMode) or fast navigations the component may mount twice.
      // We combine an in-memory ref with a sessionStorage flag so remounts do not
      // trigger duplicate /token + /userinfo exchanges that can trip rate limits.
      const processedKey = 'authentik_callback_processed';
      const alreadyPersisted = sessionStorage.getItem(processedKey) === 'true';
      if (hasProcessed.current || alreadyPersisted) {
        logger.info('OAuth callback already processed (guard active), skipping duplicate handling');
        return;
      }
      ...
      try {
        ...
        if (!code || !state) {
         ...
         logger.info('🔍 [MarcobyIAMCallback] handleOAuthCallback result', { ok: result.success });
         ...
         if (!result.success || !result.data) {
           ...
           setError(errorMessage);
           setProcessing(false);
           // Allow retry: clear guard so user can attempt again via navigation
           sessionStorage.removeItem(processedKey);
           return;
         }

         logger.info('🔍 [MarcobyIAMCallback] OAuth callback successful, refreshing auth context');
         ...
         if (isFromSignup) {
          ...
          const redirectDestination = urlNext || stateFrom || '/chat';
        ...
        setProcessing(false);
        // Clear processed flag to allow retry on hard reload or user action
        sessionStorage.removeItem(processedKey);
      }
    };

    handleCallback();
  // location is intentionally not included to avoid re-triggering on unrelated state object identity changes.
