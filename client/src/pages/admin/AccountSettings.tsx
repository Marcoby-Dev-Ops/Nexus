...
const AccountSettings: React.FC = () => {
  ...
  const { form, handleSubmit, isSubmitting, isValid, errors } = useFormWithValidation<UserProfileFormData>({
    ...
    onSubmit: async (data: UserProfileFormData) => {
      ...
      if (companyNameInput) {
        try {
          const normalizedName = companyNameInput.toLowerCase();

          if (company && company.name && company.name.toLowerCase() === normalizedName) {
            companyId = company.id;
            logger.info('Using existing company from context', { companyId, companyName: company.name });
          } else if (!companyId) {
            ...
            if (created.success && created.data) {
              ...
              logger.error('Failed to create company', { companyName: companyNameInput, error: created.error });
            }
          }
        } catch (err) {
