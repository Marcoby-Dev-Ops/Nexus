...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      (window as WindowWithRequestIdleCallback).requestIdleCallback(() => setShowCharts(true));
    } else {
  ...
  return (
    <div className="p-8 space-y-8">
          ...
          <ul className="divide-y">
            {activityFeed.map((a, i) => (
              <li key={i} className="py-2">
                <span className="font-medium">{a.user}</span> - {a.note} <span className="text-xs text-muted-foreground dark:text-gray-200">({a.date})</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 min-w-0">
        ...
        {/* Module Cards */}
        <div
          className="bg-card rounded-xl p-6 border border-border module-card transition-all duration-300 min-w-0 cursor-pointer hover:shadow-lg focus-visible:ring-2 focus-visible:ring-primary outline-none"
          tabIndex={0}
          role="button"
          aria-label="Go to Sales Dashboard"
          onClick={() => navigate('/sales')}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') navigate('/sales'); }}
        >
          <h3 className="font-semibold text-lg mb-2">Sales Dashboard</h3>
          <p className="text-muted-foreground dark:text-gray-200 text-sm mb-4">Track deals, manage pipelines, and analyze sales performance with AI-powered insights.</p>
          <button className="text-indigo-400 hover:text-indigo-300 text-sm font-medium" tabIndex={-1}>Open</button>
        </div>
        <div
          className="bg-card rounded-xl p-6 border border-border module-card transition-all duration-300 min-w-0 cursor-pointer hover:shadow-lg focus-visible:ring-2 focus-visible:ring-primary outline-none"
          tabIndex={0}
          role="button"
          aria-label="Go to Finance Hub"
          onClick={() => navigate('/finance')}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') navigate('/finance'); }}
        >
          <h3 className="font-semibold text-lg mb-2">Finance Hub</h3>
          <p className="text-muted-foreground dark:text-gray-200 text-sm mb-4">Manage invoices, expenses, and financial reports with automated reconciliation.</p>
          <button className="text-blue-400 hover:text-blue-300 text-sm font-medium" tabIndex={-1}>Open</button>
        </div>
        <div
          className="bg-card rounded-xl p-6 border border-border module-card transition-all duration-300 min-w-0 cursor-pointer hover:shadow-lg focus-visible:ring-2 focus-visible:ring-primary outline-none"
          tabIndex={0}
          ...
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') navigate('/operations'); }}
        >
          <h3 className="font-semibold text-lg mb-2">Operations Center</h3>
          <p className="text-muted-foreground dark:text-gray-200 text-sm mb-4">Streamline workflows, manage inventory, and optimize business processes.</p>
          <button className="text-yellow-400 hover:text-yellow-300 text-sm font-medium" tabIndex={-1}>Open</button>
        </div>
      </div>
