...
import { hubspotIntegration } from '@/services/integrations/hubspot';
...
const ApiDocIntegrationSetup: React.FC<ApiDocIntegrationSetupProps> = ({ onIntegrationCreated }) => {
  ...
  const generateIntegration = async () => {
    ...
    try {
      ...
      // Generate TypeScript integration code with proper imports and error handling
      const className = integrationName.replace(/[^a-zA-Z0-9]/g, '') + 'Integration';
      
      const generatedCode = `/**
 * Auto-generated integration for ${integrationName}
 * Generated by Nexus API Learning System
 */
...
${analysisResult.endpoints.map(endpoint => {
  const methodName = endpoint.name.toLowerCase().replace(/[^a-z0-9]/g, '_');
  const params = endpoint.path.match(/\\{([^}]+)\\}/g)?.map(p => p.slice(1, -1)) || [];
   ...
   */
  async ${methodName}(${paramTypes}${paramTypes ? ', ' : ''}data?: any): Promise<ApiResponse<any>> {
    try {
      const path = '${endpoint.path}'.replace(/\\{([^}]+)\\}/g, (match, param) => {
        const value = {${params.map(p => `${p}: ${p}`).join(', ')}}[param];
        return value || match;
      });
      
      return await this.client.${endpoint.method.toLowerCase()}(path${endpoint.method.toLowerCase() !== 'get' ? ', data' : ''});
    } catch (error) {
      return {
        success: false,
   ...
   */
  async getStatus(): Promise<{ status: string; lastSync?: string; errorCount: number }> {
    return {
      status: 'active',
...
export function create${className}(config: ${className}Config): ${className} {
  return new ${className}(config);
      ...
      const integrationData = {
        ...
        authMethods: analysisResult.authMethods,
      ...
      const saveResult = await consolidatedIntegrationService.saveIntegration(integrationData);
      
      if (saveResult.success && saveResult.data) {
        setIntegrationId(saveResult.data);
        setStep(4); // Success step
      } else {
        setError(`Failed to save integration: ${saveResult.error}`);
      }
