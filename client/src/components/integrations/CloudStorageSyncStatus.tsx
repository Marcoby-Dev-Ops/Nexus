...
export function CloudStorageSyncStatus({ className }: CloudStorageSyncStatusProps) {
  ...
  const loadSyncStats = async () => {
    ...
    try {
      ...
      const { database } = await import('@/shared/lib/core/database');
      const supabase = database as any;
      ...
      const { database: database2 } = await import('@/shared/lib/core/database');
      const supabase2 = database2 as any;
      const { data: lastDoc } = await supabase2
        .from('ai_vector_documents')
        .select('created_at')
        .or('document_id.like.google-drive-%,document_id.like.onedrive-%')
