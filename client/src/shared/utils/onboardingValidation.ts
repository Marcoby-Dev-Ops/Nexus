...
export class OnboardingValidationService extends BaseService {
  ...
  async validateField(fieldValue: any, rule: ValidationRule): Promise<ServiceResponse<{ isValid: boolean; message?: string }>> {
    const fieldValidation = this.validateRequiredParam(fieldValue, 'fieldValue');
    if (fieldValidation) {
      return this.createErrorResponse(fieldValidation);
    }
    ...
    return this.executeDbOperation(
      async () => {
        try {
          ...
          return { data: [], error: error instanceof Error ? error.message : 'Failed to get validation rules' };
        }
      },
      'getValidationRules'
    );
  ...
  async addValidationRule(rule: Omit<ValidationRule, 'id'>): Promise<ServiceResponse<string>> {
    const nameValidation = this.validateRequiredParam(rule.name, 'name');
    if (nameValidation) {
      return this.createErrorResponse(nameValidation);
    }

    const fieldValidation = this.validateRequiredParam(rule.field, 'field');
    if (fieldValidation) {
      return this.createErrorResponse(fieldValidation);
    }
    ...
    return this.executeDbOperation(
      async () => {
        try {
          ...
          return { data: newRule.id, error: null };
        } catch (error) {
          return { data: '', error: error instanceof Error ? error.message : 'Failed to add validation rule' };
        }
      },
      'addValidationRule'
    );
    ...
    return this.executeDbOperation(
      async () => {
        try {
          ...
          if (ruleIndex === -1) {
            return { data: null, error: 'Validation rule not found' };
          }
          ...
          return { data: null, error: error instanceof Error ? error.message : 'Failed to update validation rule' };
        }
      },
    ...
    return this.executeDbOperation(
      async () => {
        try {
          ...
          return { data: false, error: error instanceof Error ? error.message : 'Failed to delete validation rule' };
        }
      },
