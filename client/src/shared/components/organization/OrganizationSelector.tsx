...
export const OrganizationSelector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (orgs.length === 0 && !loading) {
      // Debug logging
      console.log('🔍 [OrganizationSelector] Loading memberships for user:', user.id);
      console.log('🔍 [OrganizationSelector] User object:', user);
      
      loadMemberships(user.id).catch((error) => {
        logger.error('Failed to load organization memberships', { error, userId: user.id });
        console.error('❌ [OrganizationSelector] Failed to load memberships:', error);
      });
    }
