  ...
  return (
    <div className="flex flex-col h-full bg-muted/10 border-r border-border/40">
      ...
      <div className="p-4 border-b border-border/40">
        <h2 className="text-[0.82rem] font-semibold text-foreground/75 tracking-wider uppercase mb-3">
          Exploration
        </h2>
        <div className="space-y-1">
          <Button
            variant="secondary"
            className="w-full justify-start gap-2 h-10 text-[0.95rem] font-medium bg-background/50 hover:bg-background border-border/50 shadow-sm"
          ...
          {isLoading && conversations.length === 0 ? (
            ...
            <div className="space-y-0.5">
              ...
              {timeGroups.map(group => (
                <div key={group.label}>
                  <h4 className="px-2 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider mt-4 mb-1.5">
                    {group.label}
                  </h4>
                  {group.conversations.map((conv) => (
                    <div
                      key={conv.id}
                      className={cn(
                        "group flex items-center gap-2 rounded-md px-2 py-2.5 text-[0.95rem] transition-colors cursor-pointer",
                        editingId === conv.id
