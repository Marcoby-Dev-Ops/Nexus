...
export function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const isAdminUser = (userProfile?.role === 'admin') || (_user as any)?.role === 'admin';

  // Filter nav items based on adminOnly flag when the current user is not an admin
  const overviewItems = getOverviewItems().filter((item) => {
    // item may include a custom adminOnly flag (not part of the NavItem type)
    return !((item as any).adminOnly) || isAdminUser;
  });
  const buildingBlocksItems = getBuildingBlocksItems().filter((item) => {
    return !((item as any).adminOnly) || isAdminUser;
  });
  const toolsItems = getToolsItems().filter((item) => {
    return !((item as any).adminOnly) || isAdminUser;
  });
