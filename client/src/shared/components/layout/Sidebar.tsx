...
import { X, ChevronRight, Users, Brain, HelpCircle, Home, Workflow, PieChart, Settings, BarChart2 } from 'lucide-react';
import { getCoreNavItems, getNavItemsByCategory, getToolsItems, getAdminItems } from './navConfig';
    ...
    <>
      ...
      >
        ...
        <nav className="overflow-y-auto h-[calc(100vh-4rem)]">
          {renderSection('Core Business', coreItems, <PieChart className="h-3 w-3" />)}
          {renderSection('Tools & Workflow', toolsItems, <Workflow className="h-3 w-3" />)}
          {renderSection('Admin & Settings', adminItems, <Settings className="h-3 w-3" />)}
        </nav>
      </aside>
      {isOpen && (
        <div
