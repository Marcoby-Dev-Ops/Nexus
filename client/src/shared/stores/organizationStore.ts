...
export const getOrganizations = async (userId: string): Promise<Organization[]> => {
  try {
    // Debug logging
    console.log('🔍 [OrganizationStore] Getting organizations for user ID:', userId);
    ...
    if (!sessionData) {
      ...
      console.log('❌ [OrganizationStore] No session data found');
      return [];
    }
    ...
    try {
      ...
      console.log('❌ [OrganizationStore] Failed to parse session:', parseError);
      return [];
    }
    ...
    if (!token) {
      ...
      console.log('❌ [OrganizationStore] No access token found');
      return [];
    }

    console.log('🔍 [OrganizationStore] Making API call to /api/organizations');
    ...
    console.log('🔍 [OrganizationStore] API response status:', response.status);

    if (!response.ok) {
      if (response.status === 401) {
        logger.warn('User not authenticated; returning empty organization list');
        console.log('❌ [OrganizationStore] 401 Unauthorized');
        return [];
      }
    ...
    console.log('🔍 [OrganizationStore] API response:', result);
    
    if (!result.success) {
      logger.error('API returned error', { error: result.error });
      console.log('❌ [OrganizationStore] API returned error:', result.error);
      return [];
    }

    // Reduced logging for frequently called method
    console.log('✅ [OrganizationStore] Successfully got organizations:', result.data);
    ...
    console.log('❌ [OrganizationStore] Error:', errorMessage);
    return [];
  }
};
