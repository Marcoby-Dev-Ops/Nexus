        ...
        if (sessionResult.success && sessionResult.data) {
          ...
          if (sessionResult.data.session?.accessToken) {
            try {
              ...
              console.error('❌ [MarcobyIAMContext] Failed to store session in localStorage:', error);
            }
          }
        } else {
  ...
  const signIn = async (additionalParams?: Record<string, string>): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      if (!result.success || !result.data) {
        ...
        console.error('❌ [MarcobyIAMContext] OAuth flow failed:', errorMessage);
      ...
      try {
        ...
        console.error('❌ [MarcobyIAMContext] Redirect failed:', redirectError);
        // Fallback to href
