...
export class UserService extends BaseService implements CrudServiceInterface<UserProfile> {
  ...
  /**
   * Search users (admin-only) - uses server RPC 'search_users'
   */
  async search(query?: string): Promise<ServiceResponse<UserProfile[]>> {
    return this.executeDbOperation(async () => {
      this.logMethodCall('search', { query });
      const q = query ? String(query).trim() : '';
      const rpcResult = await callRPC('search_users', { q });
      if (!rpcResult.success) {
        return { data: [], error: rpcResult.error || 'Search failed', success: false };
      }
      const data = rpcResult.data || [];
      const validated = (data as any[]).map((d) => this.config.schema.parse(d));
