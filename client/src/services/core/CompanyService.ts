...
// logger not used in this client-side service file
        ...
        try {
          ...
          if (validationError instanceof z.ZodError) {
            ...
            if (missingFields.includes('foundation') || Object.keys(result.data).length === 0) {
              ...
              // Try to use an existing company name as a sensible fallback when
              // foundation.name is missing. This avoids spamming Zod validation
              // warnings for the common case where the identity exists but the
              // foundation.name wasn't set yet.
              let fallbackName = result.data?.foundation?.name || '';
              if (!fallbackName) {
                try {
                  // selectOne returns ApiResponse<{ name: string, ... }>
                  const companyResp = await selectOne('companies', { id: companyId });
                  if (companyResp && companyResp.data && (companyResp.data as any).name) {
                    fallbackName = (companyResp.data as any).name;
                  }
                } catch {
                  // ignore lookup errors and keep fallbackName empty
                }
              }

              const defaultBusinessIdentity: BusinessIdentity = {
                foundation: {
                  name: fallbackName || 'My Company',
            ...
            // For other validation errors, ensure foundation.name is at least
            // populated with the company name if available to avoid empty-name
            // validations in the UI.
            let fallbackName2 = result.data?.foundation?.name || '';
            if (!fallbackName2) {
              try {
                const companyResp2 = await selectOne('companies', { id: companyId });
                if (companyResp2 && companyResp2.data && (companyResp2.data as any).name) {
                  fallbackName2 = (companyResp2.data as any).name;
                }
              } catch {
                // ignore
              }
