    ...
    return this.executeDbOperation(async () => {
      try {
        ...
        if (existingConnection.data) {
          ...
          const { data, error } = await updateOne('user_integrations', existingConnection.data.id, {
  ...
  async refreshAccessToken(userId: string): Promise<ServiceResponse<{ access_token: string; expires_at: string }>> {
    return this.executeDbOperation(async () => {
      try {
        ...
        const { msalInstance } = await import('@/shared/auth/msal');
        ...
        const { data, error } = await updateOne('user_integrations', connection.id, {
  ...
  private async refreshToken(userId: string): Promise<ServiceResponse<boolean>> {
    return this.executeDbOperation(async () => {
      try {
        ...
        const { error } = await updateOne('user_integrations', connection.data.id, {
