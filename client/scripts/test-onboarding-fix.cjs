...
async function testOnboardingFix() {
  ...
  try {
    ...
    const testData = {
      user_id: externalUserId, // This should work directly with the new schema
    ...
    // Test the upsert operation directly in the database with external user ID
    const upsertQuery = `
    ...
    const upsertResult = await pool.query(upsertQuery, [
      externalUserId, // Use external user ID directly
      testData.step_id,
