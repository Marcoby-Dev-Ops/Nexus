-- ====================================================================
-- MIGRATE TO CENTRALIZED RLS SYSTEM (COMPLETE)
-- This script creates the centralized RLS functions and migrates all policies
-- ====================================================================
...
CREATE OR REPLACE FUNCTION apply_user_level_policies(target_table TEXT)
RETURNS VOID AS $$
DECLARE
    has_user_id BOOLEAN;
...
BEGIN
    ...
    IF NOT has_user_id THEN
        RAISE EXCEPTION 'Table % does not have user_id column', target_table;
    END IF;
    ...
    EXECUTE format('DROP POLICY IF EXISTS "%s_service_role_policy" ON public.%I', target_table, target_table);
    
    -- Create SELECT policy
    IF has_deleted_at THEN
        EXECUTE format('CREATE POLICY "%s_select_policy" ON public.%I FOR SELECT USING (auth.uid() = user_id AND deleted_at IS NULL)', target_table, target_table);
    ELSE
        EXECUTE format('CREATE POLICY "%s_select_policy" ON public.%I FOR SELECT USING (auth.uid() = user_id)', target_table, target_table);
    END IF;
    
    -- Create INSERT policy
    EXECUTE format('CREATE POLICY "%s_insert_policy" ON public.%I FOR INSERT WITH CHECK (auth.uid() = user_id)', target_table, target_table);
    
    -- Create UPDATE policy
    IF has_deleted_at THEN
        EXECUTE format('CREATE POLICY "%s_update_policy" ON public.%I FOR UPDATE USING (auth.uid() = user_id AND deleted_at IS NULL)', target_table, target_table);
    ELSE
        EXECUTE format('CREATE POLICY "%s_update_policy" ON public.%I FOR UPDATE USING (auth.uid() = user_id)', target_table, target_table);
    END IF;
    
    -- Create DELETE policy
    IF has_deleted_at THEN
        EXECUTE format('CREATE POLICY "%s_delete_policy" ON public.%I FOR DELETE USING (auth.uid() = user_id AND deleted_at IS NULL)', target_table, target_table);
    ELSE
        EXECUTE format('CREATE POLICY "%s_delete_policy" ON public.%I FOR DELETE USING (auth.uid() = user_id)', target_table, target_table);
    END IF;
    
    -- Create service role policy (full access)
    EXECUTE format('CREATE POLICY "%s_service_role_policy" ON public.%I FOR ALL USING (auth.role() = ''service_role'')', target_table, target_table);
    
    RAISE NOTICE 'Applied user-level policies to table: %', target_table;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to apply company-level policies (for tables with company_id column)
CREATE OR REPLACE FUNCTION apply_company_level_policies(target_table TEXT)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    IF NOT has_company_id THEN
        RAISE EXCEPTION 'Table % does not have company_id column', target_table;
    END IF;
    ...
    -- Create INSERT policy
    EXECUTE format('CREATE POLICY "%s_insert_policy" ON public.%I FOR INSERT WITH CHECK (auth.role() = ''authenticated'')', target_table, target_table);
    ...
    RAISE NOTICE 'Applied company-level policies to table: %', target_table;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to apply hybrid policies (for tables with both user_id and company_id)
CREATE OR REPLACE FUNCTION apply_hybrid_policies(target_table TEXT)
RETURNS VOID AS $$
DECLARE
    has_user_id BOOLEAN;
    has_company_id BOOLEAN;
...
BEGIN
    ...
    IF NOT has_user_id OR NOT has_company_id THEN
        RAISE EXCEPTION 'Table % does not have both user_id and company_id columns', target_table;
    END IF;
    ...
    RAISE NOTICE 'Applied hybrid policies to table: %', target_table;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to apply read-only policies (for analytics and logging tables)
CREATE OR REPLACE FUNCTION apply_readonly_policies(target_table TEXT)
RETURNS VOID AS $$
BEGIN
    -- Drop existing policies
    EXECUTE format('DROP POLICY IF EXISTS "%s_select_policy" ON public.%I', target_table, target_table);
    ...
    EXECUTE format('CREATE POLICY "%s_select_policy" ON public.%I FOR SELECT USING (auth.role() = ''authenticated'')', target_table, target_table);
    ...
    RAISE NOTICE 'Applied read-only policies to table: %', target_table;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- ====================================================================
-- STEP 4: APPLY CENTRALIZED POLICIES
-- ====================================================================
...
-- ====================================================================
-- STEP 5: SPECIAL HANDLING FOR SPECIFIC TABLES
-- ====================================================================
...
BEGIN
    ...
    RAISE NOTICE '- apply_user_level_policies(table_name)';
    RAISE NOTICE '- apply_company_level_policies(table_name)';
    RAISE NOTICE '- apply_hybrid_policies(table_name)';
    RAISE NOTICE '- apply_readonly_policies(table_name)';
END $$;
