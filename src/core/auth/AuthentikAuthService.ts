 ...
 * 
 * This service handles authentication with Authentik OAuth2 provider.
 * Replaces the Supabase auth service for the migration to Authentik.
 */
 ...
 * Authentik Authentication Service
 */
export class AuthentikAuthService extends BaseService {
  ...
  private storeSession(session: AuthentikSession): void {
    try {
      ...
      console.log('✅ [AuthentikAuthService] Session stored successfully');
    } catch (error) {
      console.error('❌ [AuthentikAuthService] Failed to store session:', error);
  ...
  private getStoredSession(): AuthentikSession | null {
    try {
      ...
      console.error('❌ [AuthentikAuthService] Failed to get stored session:', error);
