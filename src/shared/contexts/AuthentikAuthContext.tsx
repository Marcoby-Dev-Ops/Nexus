...
import { authentikAuthService, type AuthUser, type AuthSession } from '@/core/auth/AuthentikAuthService';
...
const authLogger = {
  info: (message: string, data?: unknown) => {
    logger.info(`[AuthentikAuthContext] ${message}`, data);
  },
  error: (message: string, error?: unknown) => {
    logger.error(`[AuthentikAuthContext Error] ${message}`, error);
  }
};
...
export function AuthentikAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeAuth = useCallback(async () => {
    ...
    try {
      authLogger.info('Initializing Authentik auth...');
      ...
      if (authResult.success && authResult.data) {
        ...
        if (sessionResult.success && sessionResult.data) {
          ...
          console.log('🔍 [AuthentikAuthContext] Session data:', {
            hasSession: !!sessionResult.data.session,
            hasAccessToken: !!sessionResult.data.session?.accessToken,
            hasUser: !!sessionResult.data.session?.user,
            userId: sessionResult.data.session?.user?.sub
          });
          ...
          if (sessionResult.data.session?.accessToken) {
            try {
              ...
              if (!existingSession) {
                ...
                console.log('✅ [AuthentikAuthContext] Manually stored session in localStorage');
              } else {
                console.log('ℹ️ [AuthentikAuthContext] Session already exists in localStorage');
              }
            } catch (error) {
              console.error('❌ [AuthentikAuthContext] Failed to store session in localStorage:', error);
            }
          }
        } else {
  ...
  const signIn = async (additionalParams?: Record<string, string>): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      if (!result.success || !result.data) {
        ...
        console.error('❌ [AuthentikAuthContext] OAuth flow failed:', errorMessage);
      ...
      authLogger.info('Redirecting to Authentik:', result.data);
      console.log('🔍 [AuthentikAuthContext] Redirecting to:', result.data);
      
      // Try using replace instead of href for more reliable redirect
      try {
        console.log('🔍 [AuthentikAuthContext] Attempting window.location.replace...');
        window.location.replace(result.data);
        console.log('🔍 [AuthentikAuthContext] window.location.replace completed');
      } catch (redirectError) {
        console.error('❌ [AuthentikAuthContext] Redirect failed:', redirectError);
        // Fallback to href
        console.log('🔍 [AuthentikAuthContext] Falling back to window.location.href...');
