...
import { OnboardingDebugMonitor } from '@/components/dev/OnboardingDebugMonitor';
...
interface OnboardingStepProps {
  ...
  currentStep: number;
  totalSteps: number;
  user?: Record<string, unknown>; // Pass user from parent to avoid multiple useUser calls
}
...
const CoreIdentityStep: React.FC<OnboardingStepProps> = ({ onNext, onSkip, data, currentStep, totalSteps, user }) => {
  ...
  const [company, setCompany] = useState((data.company as string) || '');
  const [industry, setIndustry] = useState((data.industry as string) || '');
  const [companySize, setCompanySize] = useState((data.companySize as string) || '');
  const [keyPriorities, setKeyPriorities] = useState<string[]>((data.keyPriorities as string[]) || []);
  ...
  }, [user?.id, firstName, lastName, email, jobTitle, phone, company, industry, companySize, keyPriorities, saveStep, onNext]);

  const canProceed = firstName.trim() && lastName.trim() && email.trim() && company.trim() && industry && companySize && keyPriorities.length > 0;
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="flex justify-center gap-4">
        ...
        <Button 
          ...
          className="px-6 md:px-8 py-3 text-base md:text-lg bg-primary text-primary-foreground hover:bg-primary/90"
        >
          {isProcessing ? 'Saving...' : 'Next: Your Business Tools'}
        </Button>
      </div>
    </div>
  );
...
// Step 2: Tool Identification by Building Block
const ToolIdentificationStep: React.FC<OnboardingStepProps> = ({ onNext, onSkip, data, currentStep, totalSteps, user }) => {
  const { saveStep, isProcessing } = useOnboardingService();
  const [selectedTools, setSelectedTools] = useState<{[blockId: string]: string[]}>(data.selectedTools || {});
  const [expandedBlock, setExpandedBlock] = useState<string | null>(null);

  // 7 Building Blocks with their associated tools
  const buildingBlocks = [
    {
      ...
      tools: [
        ...
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    },
    {
      id: 'revenue',
      name: 'Revenue',
      description: 'Customers, deals, contracts, subscriptions, and sales pipeline',
      icon: '💰',
      tools: [
        { id: 'salesforce', name: 'Salesforce', description: 'Enterprise CRM' },
        { id: 'hubspot', name: 'HubSpot', description: 'CRM and marketing' },
        { id: 'pipedrive', name: 'Pipedrive', description: 'Sales CRM' },
        { id: 'zoho-crm', name: 'Zoho CRM', description: 'Customer relationship management' },
        { id: 'stripe', name: 'Stripe', description: 'Payment processing' },
        { id: 'paypal', name: 'PayPal', description: 'Payment processing' },
        { id: 'square', name: 'Square', description: 'Payment and POS' },
        { id: 'linkedin-sales', name: 'LinkedIn Sales Navigator', description: 'Sales prospecting' },
    ...
    },
    {
      id: 'cash',
      name: 'Cash',
      description: 'Invoices, expenses, payroll, taxes, and financial controls',
      icon: '💳',
      tools: [
        { id: 'quickbooks', name: 'QuickBooks', description: 'Accounting software' },
        { id: 'xero', name: 'Xero', description: 'Cloud accounting' },
        { id: 'freshbooks', name: 'FreshBooks', description: 'Accounting and invoicing' },
        { id: 'wave', name: 'Wave', description: 'Free accounting software' },
        { id: 'gusto', name: 'Gusto', description: 'Payroll and HR' },
        { id: 'bamboo-hr', name: 'BambooHR', description: 'HR management' },
        { id: 'expensify', name: 'Expensify', description: 'Expense management' },
        { id: 'mint', name: 'Mint', description: 'Personal finance tracking' },
        { id: 'microsoft365', name: 'Microsoft 365', description: 'Business applications and Excel' },
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    },
    {
      ...
      tools: [
        ...
        { id: 'quickbooks-commerce', name: 'QuickBooks Commerce', description: 'Inventory management' },
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    },
    {
      id: 'people',
      name: 'People',
      description: 'Employees, contractors, teams, skills, performance, and culture',
      icon: '👥',
      tools: [
        { id: 'slack', name: 'Slack', description: 'Team communication' },
        { id: 'teams', name: 'Microsoft Teams', description: 'Team collaboration' },
        { id: 'zoom', name: 'Zoom', description: 'Video conferencing' },
        { id: 'google-meet', name: 'Google Meet', description: 'Video meetings' },
        { id: 'toggl', name: 'Toggl', description: 'Time tracking' },
        { id: 'harvest', name: 'Harvest', description: 'Time tracking and invoicing' },
        { id: '15five', name: '15Five', description: 'Performance management' },
        { id: 'lattice', name: 'Lattice', description: 'Performance and engagement' },
        { id: 'microsoft365', name: 'Microsoft 365', description: 'Collaboration and productivity' },
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    },
    {
      id: 'knowledge',
      name: 'Knowledge',
      description: 'Data, insights, learning, and documentation',
      icon: '📚',
      tools: [
        { id: 'google-drive', name: 'Google Drive', description: 'Cloud storage' },
        { id: 'dropbox', name: 'Dropbox', description: 'File storage' },
        { id: 'onedrive', name: 'OneDrive', description: 'Microsoft cloud storage' },
        { id: 'confluence', name: 'Confluence', description: 'Team workspace' },
        { id: 'evernote', name: 'Evernote', description: 'Note taking' },
        { id: 'roam-research', name: 'Roam Research', description: 'Knowledge management' },
        { id: 'obsidian', name: 'Obsidian', description: 'Knowledge base' },
        { id: 'guru', name: 'Guru', description: 'Knowledge management' },
        { id: 'scribe-ai', name: 'Scribe AI', description: 'AI-powered documentation' },
        { id: 'microsoft365', name: 'Microsoft 365', description: 'Document storage and collaboration' },
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    },
    {
      id: 'systems',
      name: 'Systems',
      description: 'Tools, processes, automation, and technology infrastructure',
      icon: '⚙️',
      tools: [
        { id: 'zapier', name: 'Zapier', description: 'Workflow automation' },
        { id: 'make', name: 'Make (Integromat)', description: 'Automation platform' },
        { id: 'n8n', name: 'N8N', description: 'Workflow automation' },
        { id: 'ifttt', name: 'IFTTT', description: 'App automation' },
        { id: 'google-analytics', name: 'Google Analytics', description: 'Website analytics' },
        ...
        { id: 'jira', name: 'Jira', description: 'Project and issue tracking' },
        { id: 'nocodb', name: 'NocoDB', description: 'Database and no-code platform' },
        { id: 'coolify', name: 'Coolify', description: 'Self-hosted deployment platform' },
        { id: 'ninjaone', name: 'NinjaOne', description: 'RMM and IT management' },
        { id: 'sentinelone', name: 'SentinelOne', description: 'Cybersecurity platform' },
        { id: 'none', name: 'None yet', description: 'I don\'t have tools for this yet' }
      ]
    }
  ];

  const handleToolToggle = (blockId: string, toolId: string) => {
    setSelectedTools(prev => {
      const currentTools = prev[blockId] || [];
      
      // If selecting "none", clear all other selections for this block
      if (toolId === 'none') {
        return { ...prev, [blockId]: ['none'] };
      }
      
      // If selecting a tool other than "none", remove "none" if it exists
      let updatedTools = currentTools.filter(id => id !== 'none');
      ...
      if (currentTools.includes(toolId)) {
        ...
        updatedTools = [...updatedTools, toolId];
      }
      
      return { ...prev, [blockId]: updatedTools };
    });
  };

  const getSelectedCount = (blockId: string) => {
    const tools = selectedTools[blockId] || [];
    return tools.filter(id => id !== 'none').length;
  };
  ...
  const handleNext = async () => {
    const stepData = {
      ...data,
      selectedTools,
      toolIdentificationCompleted: true
    };

    const success = await saveStep('tool-identification', stepData);
  ...
  const totalSelectedTools = Object.values(selectedTools).flat().filter(id => id !== 'none').length;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Your Business Tools</h2>
        <p className="text-gray-600">
          Tell us what tools you're already using across your business
        </p>
      </div>
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-500">
                  {getSelectedCount(block.id)} selected
                </span>
                <svg
              ...
              <div className="p-4 bg-white border-t border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {block.tools.map((tool) => {
                    const isSelected = (selectedTools[block.id] || []).includes(tool.id);
                    return (
                      <label
                        key={tool.id}
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => handleToolToggle(block.id, tool.id)}
      ...
      {totalSelectedTools > 0 && (
        <div className="p-4 bg-blue-50 rounded-lg mb-6">
          <h4 className="font-medium text-blue-900 mb-2">Great! We found {totalSelectedTools} tools</h4>
          <div className="text-sm text-blue-800">
            <p>• We'll help you connect these tools to get insights across your business</p>
            <p>• Your dashboard will be personalized based on your current setup</p>
            <p>• We'll identify gaps and recommend tools to strengthen weak areas</p>
          </div>
        </div>
      )}
...
const DayOneInsightStep: React.FC<OnboardingStepProps> = ({ onNext, onSkip, onBack, data, currentStep, totalSteps, user }) => {
  const { saveStep, isProcessing } = useOnboardingService();
  const [maturityScore, setMaturityScore] = useState(data.maturityScore || 45);
  const [insights, setInsights] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [insightFeedback, setInsightFeedback] = useState<Record<string, 'agree' | 'disagree' | null>>({});
  const [savingFireInitiatives, setSavingFireInitiatives] = useState(false);
  const [alternativeInsights, setAlternativeInsights] = useState<any[]>([]);

  // Generate AI-powered insights based on business context
  useEffect(() => {
    const generateAIInsights = async () => {
      ...
      try {
        ...
        // Import the client service that calls server-side API
        const { onboardingInsightsClient } = await import('@/services/ai/OnboardingInsightsClient');
        ...
        const context = {
          ...
          maturityScore: data.maturityScore || 45
        };

        // Debug: Log the context being sent to AI
        logger.info('🔍 AI Insights Debug - Context being sent:', context);

        const result = await onboardingInsightsClient.generateOnboardingInsights(context);
        
        if (result.success && result.data) {
          setInsights(result.data);
          // Update maturity score based on AI analysis
          const calculatedScore = onboardingInsightsClient.calculateMaturityScore(context, result.data);
          setMaturityScore(calculatedScore);
          
                  // Debug: Log the insights received
        logger.info('🔍 AI Insights Debug - Insights received:', result.data);
        } else {
        ...
        const fallbackInsights = [
          {
            ...
            reasoning: 'Industry benchmark analysis shows significant revenue impact',
          ...
          },
          {
            ...
            description: 'Automating invoice processing could save your team 8-12 hours per week on administrative tasks.',
            impact: 'Medium',
            confidence: 92,
            action: 'Connect your accounting software for automated workflows',
            reasoning: 'Manual invoice processing is a common time sink',
          ...
          },
          {
            ...
            description: 'Your industry has a 15% higher customer churn rate than average. Implementing customer success tracking can improve retention by 25%.',
            impact: 'High',
            confidence: 78,
            action: 'Implement customer success tracking and engagement monitoring',
            reasoning: 'Customer retention is critical for sustainable growth',
        ...
        // Add integration-specific fallback insights
        if (data.selectedIntegrations?.includes('hubspot')) {
          fallbackInsights.push({
            id: 'fallback-4',
            type: 'integration',
            title: 'HubSpot CRM Optimization',
            description: 'Your HubSpot integration provides access to advanced lead scoring and marketing automation features.',
            impact: 'Medium',
            confidence: 95,
            action: 'Configure HubSpot lead scoring and marketing automation workflows',
            reasoning: 'HubSpot has powerful features that are often underutilized',
        ...
        if (data.selectedIntegrations?.includes('quickbooks')) {
          fallbackInsights.push({
            ...
            description: 'Your QuickBooks integration enables real-time financial monitoring and automated reporting.',
            impact: 'Medium',
            confidence: 88,
            action: 'Set up automated financial reports and cash flow monitoring',
            reasoning: 'Regular financial monitoring is essential for business health',
    ...
    generateAIInsights();
  }, [data.selectedIntegrations, data.selectedTools, data.industry, data.keyPriorities, user?.id]);

  // Handle user feedback on insights
  const handleInsightFeedback = async (insightId: string, feedback: 'agree' | 'disagree') => {
    setInsightFeedback(prev => ({ ...prev, [insightId]: feedback }));

    if (feedback === 'agree') {
      // Save as FIRE initiative
      setSavingFireInitiatives(true);
      try {
        const insight = insights.find(i => i.id === insightId);
        if (insight && user?.id) {
          const { FireInitiativeService } = await import('@/services/ai/FireInitiativeService');
          const success = await FireInitiativeService.saveAsFireInitiative(
            insight, 
            user.id, 
            user.company_id || ''
          );
          ...
          if (success) {
            console.log(`✅ Saved "${insight.title}" as FIRE initiative`);
          } else {
            console.error(`❌ Failed to save "${insight.title}" as FIRE initiative`);
          }
        }
      } catch (error) {
        console.error('Error saving FIRE initiative:', error);
      } finally {
        setSavingFireInitiatives(false);
      }
    } else if (feedback === 'disagree') {
      // Replace with alternative insight
      if (alternativeInsights.length === 0) {
        const { FireInitiativeService } = await import('@/services/ai/FireInitiativeService');
        const alternatives = FireInitiativeService.getAlternativeInsights({
          user: {
            firstName: data.firstName || user?.first_name || '',
            lastName: data.lastName || user?.last_name || '',
            company: data.company || '',
        ...
        setAlternativeInsights(alternatives);
      }

      // Replace the disagreed insight with a random alternative
      const disagreedInsightIndex = insights.findIndex(i => i.id === insightId);
      if (disagreedInsightIndex !== -1 && alternativeInsights.length > 0) {
        const randomAlternative = alternativeInsights[Math.floor(Math.random() * alternativeInsights.length)];
        const newInsights = [...insights];
        newInsights[disagreedInsightIndex] = randomAlternative;
        setInsights(newInsights);
        
        // Remove the used alternative
        setAlternativeInsights(prev => prev.filter(alt => alt.id !== randomAlternative.id));
      }
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* Maturity Score */}
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-secondary/10 border border-primary/20 rounded-lg p-6 mb-8 shadow-sm">
        <div className="text-center">
          <h3 className="text-lg font-semibold text-foreground mb-2">Your Business Maturity Score</h3>
          <div className="relative inline-block">
            <div className="w-24 h-24 rounded-full border-4 border-primary/30 flex items-center justify-center bg-card/50">
              <span className="text-2xl font-bold text-primary">{maturityScore}</span>
            </div>
            <div className="absolute inset-0 w-24 h-24 rounded-full border-4 border-primary border-t-transparent transform rotate-45"></div>
          </div>
      ...
      {loading && (
        <div className="space-y-4 mb-8">
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-600 mt-2">Analyzing your business context and generating personalized recommendations</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-8">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">AI Insights Temporarily Unavailable</h3>
       ...
       {!loading && !error && (
         <div className="space-y-4 mb-8">
           ...
           <p className="text-sm text-gray-600 mb-4">
             Review these insights and let us know if they're valuable for your business. 
             Valuable insights will be saved as FIRE initiatives for you to track and implement.
           </p>
          ...
          <div key={insight.id} className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-100">
              <div className="flex items-center space-x-2">
                {insightFeedback[insight.id] === 'agree' ? (
                  <span className="text-sm text-green-600 font-medium flex items-center">
                    <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    </svg>
                    Saved as FIRE Initiative
                  </span>
                ) : insightFeedback[insight.id] === 'disagree' ? (
                  ...
                  <>
                    <button
                      ...
                      disabled={savingFireInitiatives}
                      className="flex items-center px-3 py-1.5 text-sm font-medium text-green-700 bg-green-100 border border-green-200 rounded-md hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {savingFireInitiatives ? (
                    ...
                    <button
                      ...
                      className="flex items-center px-3 py-1.5 text-sm font-medium text-red-700 bg-red-100 border border-red-200 rounded-md hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-500"
                    >
                      <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                      </svg>
              ...
              {insight.estimatedValue && (
                <div className="text-sm text-gray-600">
                  <span className="font-medium">Estimated Value:</span> {insight.estimatedValue}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-8">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span>FIRE Initiatives Created</span>
            <span className="text-green-600">
              {Object.values(insightFeedback).filter(feedback => feedback === 'agree').length}
            </span>
          </div>
        </div>
      </div>

      {/* FIRE Initiatives Summary */}
      {Object.values(insightFeedback).some(feedback => feedback === 'agree') && (
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-green-800">
                FIRE Initiatives Created Successfully!
              </h3>
              <div className="mt-2 text-sm text-green-700">
                <p>
                  {Object.values(insightFeedback).filter(feedback => feedback === 'agree').length} valuable insight(s) 
                  have been saved as FIRE initiatives. You can track and implement these in your dashboard.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="text-center mb-8">
        <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-purple-100 mb-4">
          <svg className="h-8 w-8 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2zm0 0a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2H9a2 2 0 00-2 2v10a2 2 0 002 2z" />
          </svg>
        </div>
...
const BusinessContextStep: React.FC<OnboardingStepProps> = ({ onNext, _onSkip, data, currentStep, totalSteps, user }) => {
  const { saveStep, isProcessing } = useOnboardingService();
...
// FIRE Concepts Introduction Step
const FIREConceptsIntroductionStep: React.FC<OnboardingStepProps> = ({ onNext, onSkip, data, currentStep, totalSteps }) => {
  const [discoveredOpportunities, setDiscoveredOpportunities] = useState<any[]>([]);
  const [isLoadingOpportunities, setIsLoadingOpportunities] = useState(false);
  const [selectedOpportunities, setSelectedOpportunities] = useState<string[]>([]);

  // Load discovered opportunities based on business context
  const loadDiscoveredOpportunities = async () => {
    setIsLoadingOpportunities(true);
    
    try {
      // Get existing FIRE initiatives from the database
      const { data: existingThoughts } = await supabase
        .from('thoughts')
        .select('*')
        .eq('category', 'fire_initiative')
        .eq('user_id', data.userId || 'onboarding')
        .order('created_at', { ascending: false })
        .limit(6);

      if (existingThoughts && existingThoughts.length > 0) {
        setDiscoveredOpportunities(existingThoughts);
      } else {
        // Generate sample opportunities based on business context
        const sampleOpportunities = generateSampleOpportunities(data);
        setDiscoveredOpportunities(sampleOpportunities);
      }
    } catch (error) {
      console.error('Error loading opportunities:', error);
      // Fallback to sample opportunities
      const sampleOpportunities = generateSampleOpportunities(data);
      setDiscoveredOpportunities(sampleOpportunities);
    } finally {
  ...
  // Generate sample opportunities based on business context
  const generateSampleOpportunities = (context: any) => {
    const industry = context.industry || 'Technology';
    const companySize = context.companySize || 'Small';
      ...
      <div className="text-center mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          <div className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
            ...
            <p className="text-sm text-blue-800">Quick wins and rapid implementation cycles</p>
          </div>
          <div className="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
            <div className="text-green-600 font-semibold mb-2">💥 Impactful</div>
            <p className="text-sm text-green-800">Measurable business value and outcomes</p>
          </div>
          <div className="p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
            <div className="text-purple-600 font-semibold mb-2">🎯 Relevant</div>
            <p className="text-sm text-purple-800">Aligned with your specific business context</p>
          </div>
          <div className="p-4 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg border border-orange-200">
            <div className="text-orange-600 font-semibold mb-2">⚡ Executable</div>
            <p className="text-sm text-orange-800">Actionable with clear implementation paths</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-12">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Based on your business context, Nexus has identified key opportunities that follow the FIRE philosophy. 
          ...
          <CardContent>
            {isLoadingOpportunities ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">Analyzing your business context...</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        opportunity.impact === 'High' ? 'bg-orange-100 text-orange-800' :
...
const InitialThoughtsGenerationStep: React.FC<OnboardingStepProps> = ({ onNext, onSkip, data, currentStep, totalSteps }) => {
  ...
  const generateInitialThoughts = async () => {
    ...
    try {
      ...
      const unifiedBrain = nexusUnifiedBrain;
                ...
                {aiRecommendations.map((recommendation) => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      <Badge variant={recommendation.impact === 'High' ? 'destructive' : 'secondary'} className="text-xs">
        ...
        {(selectedGoals.length > 0 || customGoals.length > 0) && (
          <Card className="mb-6 bg-primary/5 border-primary/20">
            <CardContent className="pt-6">
              <h4 className="font-medium text-primary mb-2">Goals Summary</h4>
...
const IndustrySelectionStep: React.FC<OnboardingStepProps> = ({ onNext, _onSkip, data, _currentStep, _totalSteps }) => {
...
const GoalDefinitionStep: React.FC<OnboardingStepProps> = ({ onNext, _onSkip, data, _currentStep, _totalSteps }) => {
...
const ChallengeIdentificationStep: React.FC<OnboardingStepProps> = ({ onNext, _onSkip, data, _currentStep, _totalSteps }) => {
...
const ThreePhaseOnboardingFlow: React.FC<{ 
  ...
  const stepComponents: Record<string, React.ComponentType<OnboardingStepProps>> = {
    ...
    'ToolIdentificationStep': ToolIdentificationStep,
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      ...
      // Notify parent component of data changes for debug monitor
      onDataChange?.(formData);
    }
  }, [formData, saveToLocalStorage, onDataChange]);
  ...
  const loadOnboardingProgress = async (uid: string) => {
    ...
    try {
      ...
      const progressResult = await getOnboardingProgress(uid);
  ...
  const handleStepBack = async () => {
    if (!currentPhaseData || !currentStepData) {
      logger.warn('Cannot go back: missing phase or step data');
      return;
    }

    const currentStepIndex = currentPhaseData.steps.findIndex((s: any) => s.id === currentStepData.id);
    
    if (currentStepIndex > 0) {
      // Go to previous step in current phase
      const previousStep = currentPhaseData.steps[currentStepIndex - 1];
      setCurrentStepData(previousStep);
      logger.info('Moved to previous step', { previousStep: previousStep.id });
    } else if (currentPhase > 1) {
      // Go to previous phase's last step
      try {
        const previousPhaseConfig = await getPhaseConfiguration(currentPhase - 1);
        if (previousPhaseConfig && previousPhaseConfig.success && previousPhaseConfig.data) {
          setCurrentPhaseData(previousPhaseConfig.data);
          const lastStep = previousPhaseConfig.data.steps[previousPhaseConfig.data.steps.length - 1];
          setCurrentStepData(lastStep);
          logger.info('Moved to previous phase', { 
            previousPhase: previousPhaseConfig.data.id,
            lastStep: lastStep.id 
          });
        }
      } catch (error) {
        logger.error('Error navigating to previous phase:', error);
      }
    } else {
      logger.warn('Already at the first step of the first phase');
    }
  };
    ...
    try {
      ...
      if (isLastStepInPhase) {
        ...
        try {
          ...
          if (phaseResult && !phaseResult.error && (phaseResult.nextPhase || phaseResult.data?.nextPhase)) {
            // Persist progress immediately
            const nextPhase = phaseResult.nextPhase || phaseResult.data?.nextPhase;
            const nextStep = phaseResult.nextStep || phaseResult.data?.nextStep;
            
            saveToLocalStorage(STORAGE_KEYS.PROGRESS, {
              currentPhase: nextPhase || currentPhaseData.id,
            ...
            if (nextPhase) {
              logger.info('Moving to next phase', { nextPhase });
              const nextPhaseConfig = await getPhaseConfiguration(nextPhase);
...
export const AppWithOnboarding = React.memo<AppWithOnboardingProps>(({ children }) => {
  ...
  const [currentOnboardingData, setCurrentOnboardingData] = useState<any>({});
  
  // Debug: Log onboarding data changes
  useEffect(() => {
    if (Object.keys(currentOnboardingData).length > 0) {
      console.log('🔍 Debug: Onboarding data updated:', currentOnboardingData);
    }
  ...
  if (showOnboarding && user) {
    return (
      <div className="min-h-screen bg-background">
        <ThreePhaseOnboardingFlow 
          onComplete={handleOnboardingComplete} 
          onDataChange={setCurrentOnboardingData}
        />
        <OnboardingDebugMonitor 
          debugData={{
            selectedIntegrations: currentOnboardingData.selectedIntegrations || [],
            selectedTools: currentOnboardingData.selectedTools || {},
            industry: currentOnboardingData.industry || '',
            company: currentOnboardingData.company || '',
            keyPriorities: currentOnboardingData.keyPriorities || [],
            insightsCount: currentOnboardingData.insights?.length || 0,
            maturityScore: (() => {
              // Calculate maturity score in real-time
              let score = 45; // Base score
              
              // Add points for integrations
              const integrationCount = (currentOnboardingData.selectedIntegrations || []).length;
              score += Math.min(integrationCount * 5, 20);
              
              // Add points for tool coverage
              const toolCategories = Object.keys(currentOnboardingData.selectedTools || {}).length;
              score += Math.min(toolCategories * 3, 15);
              
              // Add points for industry alignment
              if (currentOnboardingData.industry && currentOnboardingData.industry !== 'Other') {
                score += 5;
              }
              
              // Add points for clear priorities
              if (currentOnboardingData.keyPriorities?.length > 0) {
                score += Math.min(currentOnboardingData.keyPriorities.length * 2, 10);
              }
              
              return Math.min(100, Math.max(0, score));
            })(),
            currentStep: 'onboarding',
            completedSteps: currentOnboardingData.completedSteps || []
          }}
          onRefresh={() => {
            // Refresh debug data by re-checking onboarding status
            handleCompletionCheckerComplete();
          }}
        />
      </div>
    );
