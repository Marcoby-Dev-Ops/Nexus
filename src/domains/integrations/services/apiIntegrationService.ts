// src/integrations/lib/services/apiIntegrationService.ts
...
export const ApiIntegrationService = {
  ...
  async generateTypesFromOpenApi(apiDoc: string): Promise<string> {
    // Accepts a JSON string (OpenAPI spec)
    try {
      const schema = JSON.parse(apiDoc);
      const types = await openapiTS(schema, { commentHeader: false });
      return types; // This is a string of TypeScript type definitions
    } catch (e) {
      throw new Error('Failed to generate types from OpenAPI doc: ' + (e as Error).message);
    }
  },
