...
class OWAInboxService {
  ...
  private enrichEmailData(email: any, provider: EmailProvider = 'microsoft'): OWAEmailItem {
    const enriched = {
      ...
      sender_email: email.from?.emailAddress?.address || email.from?.address,
      sender_name: email.from?.emailAddress?.name || email.from?.name,
      body_preview: email.bodyPreview || email.snippet,
      item_timestamp: email.receivedDateTime || email.internalDate,
      is_read: email.isRead !== undefined ? email.isRead : !email.labelIds?.includes('UNREAD'),
      is_important: email.importance === 'high' || email.labelIds?.includes('IMPORTANT'),
      has_attachments: email.hasAttachments || email.payload?.parts?.some((part: any) => part.filename),
      thread_id: email.conversationId || email.threadId,
  ...
  private analyzeComplianceAndRisk(email: OWAEmailItem): ComplianceAnalysis {
    ...
    return {
      ...
      sentiment_score: Math.max(-100, Math.min(sentimentScore, 100)),
      urgency_score: Math.min(urgencyScore, 100),
      ai_insights: insights,
      data_classification: dataClassification,
             retention_guidance: this.getRetentionGuidance(dataClassification, riskScore),
    };
  }
   ...
   */
  private getRetentionGuidance(classification: string, _riskScore: number): string {
    switch (classification) {
      ...
      default:
        return 'Standard retention applies';
    }
  }
  ...
  private async ensureUserIntegration(userId: string, provider: EmailProvider): Promise<void> {
    try {
      ...
      const providerConfig = this.getProviderConfig(provider);
          ...
          .insert({
            user_id: userId,
            integration_id: '00000000-0000-0000-0000-000000000000', // Placeholder UUID
            integration_type: 'oauth', // Use 'oauth' for OAuth-based integrations
            integration_name: providerConfig.name,
            status: 'connected',
   ...
   */
  private async fetchGmailEmails(filters: OWAInboxFilters, limit: number, _offset: number): Promise<OWAEmailItem[]> {
