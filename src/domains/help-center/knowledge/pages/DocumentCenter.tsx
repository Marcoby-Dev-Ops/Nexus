...
import { FileText, Upload, Search, Download, Eye, Trash2, Star, Share2, FolderPlus, Grid3X3, List, Calendar, Brain, Zap, CheckCircle2, FileImage, FileVideo, Archive, ExternalLink, Folder, File } from 'lucide-react';
...
const DocumentCenter: React.FC = () => {
  ...
  const loadStats = async () => {
    try {
      const { data, error } = await supabase
        .rpc('get_document_stats', { puser_id: user?.id });
    ...
    console.warn('AI processing failed: ', error);
        ...
        const { data: docData, error: docError } = await supabase
          ...
          .insert({
            ...
            previewavailable: file.type.includes('image') || file.type.includes('pdf')
          })
          .select()
          .single();
  ...
  const toggleStar = async (documentId: string) => {
    try {
      ...
      const { error } = await supabase
        .from('ai_documents')
        .update({ isstarred: !doc.is_starred })
        .eq('id', documentId);

      if (error) throw error;

      setDocuments(prev => prev.map(d => 
        d.id === documentId ? { ...d, isstarred: !d.is_starred } : d
      ));
    } catch (error) {
      // eslint-disable-next-line no-console
    // eslint-disable-next-line no-console
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          <Star className={`h-3 w-3 ${doc.is_starred ? 'fill-warning text-warning' : 'text-muted-foreground'}`} />
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
                ...
                <div key={doc.id} className="flex items-center justify-between p-4 rounded-lg hover: bg-muted/50">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() => toggleStar(doc.id)}
                      aria-label={doc.is_starred ? 'Remove from starred' : 'Add to starred'}
                    >
                      <Star className={`h-4 w-4 ${doc.is_starred ? 'fill-warning text-warning' : 'text-muted-foreground'}`} />
                    </Button>
                    <Button
                      variant="ghost"
                      size="sm"
        ...
        <Dialog open={!!selectedDocument} onOpenChange={() => setSelectedDocument(null)}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedDocument.type} &mdash; {formatFileSize(selectedDocument.size)}
              </DialogDescription>
            </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <Star className={`h-4 w-4 mr-2 ${selectedDocument.is_starred ? 'fill-warning text-warning' : ''}`} />
