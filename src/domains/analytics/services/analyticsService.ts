...
class AnalyticsService {
  ...
  async trackEventImmediate(
    ...
    try {
      ...
      // Track immediately to analytics_events table
      if (this.analyticsTableExists) {
        const { error } = await supabase
          .from('analytics_events')
          .insert({
        ...
        if (error) {
          logger.error({ error, eventType: event.event_type }, 'Failed to track immediate analytics event');
        } else {
          logger.debug({ eventType: event.event_type }, 'Analytics event tracked immediately');
        }
      } else {
        logger.debug({ eventType: event.event_type }, 'Analytics event tracked (fallback mode)');
      }
    } catch (error) {
