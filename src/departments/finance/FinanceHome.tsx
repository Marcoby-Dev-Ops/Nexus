          ...
          <ul className="divide-y">
            {recentExpenses.map((e, i) => (
              <li key={i} className="py-2">
                <span>{e.date}</span> - <span>{e.category}</span> <span className="text-xs text-muted-foreground">${e.amount}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="rounded-xl border p-4">
        ...
        <ul className="divide-y">
          {activityFeed.map((a, i) => (
            <li key={i} className="py-2">
              {a.type === 'payment' ? '💸' : '🧾'} {a.note} <span className="text-xs text-muted-foreground">({a.date})</span>
            </li>
          ))}
        </ul>
      </div>
