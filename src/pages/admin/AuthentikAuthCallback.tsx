...
export default function AuthentikAuthCallback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      logger.info('🔍 [AuthentikAuthCallback] Callback component mounted, processing...');
      logger.info('🔍 [AuthentikAuthCallback] Current URL:', window.location.href);
      logger.info('🔍 [AuthentikAuthCallback] Search params:', Object.fromEntries(searchParams.entries()));
      ...
      try {
        logger.info('Processing Authentik OAuth callback...');
        ...
        if (!code || !state) {
         ...
         logger.info('🔍 [AuthentikAuthCallback] Calling handleOAuthCallback with code and state');
         const result = await authentikAuthService.handleOAuthCallback(code, state);
         
         logger.info('🔍 [AuthentikAuthCallback] handleOAuthCallback result:', { 
         ...
         logger.info('🔍 [AuthentikAuthCallback] OAuth callback successful, refreshing auth state', { 
           userId: result.data.id 
         });
         
         // Refresh the authentication context (only once)
         logger.info('🔍 [AuthentikAuthCallback] Calling refreshAuth...');
         await refreshAuth();
